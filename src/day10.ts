 ...
 * 0 (the first element in the list), a skip size (which starts at 0), and a sequence of lengths
 ...
 * The current position moves forward by the length plus the skip size, a total of 5, causing it
 * not to move because it wraps around: 4 3 0 [1] 2. The skip size increases to 2.
 ...
 * @returns {number}
 */
function task1(input:string = day10input, listSize:number = 256):number {
  const inputs:number[] = input.split(',').map(v => +v);
  const list = runner(listSize, inputs);
 ...
 * the dense hash. To do this, use numeric bitwise XOR to combine each consecutive block of 16
 * numbers in the sparse hash (there are 16 such blocks in a list of 256 numbers). So, the first
 * element in the dense hash is the first sixteen elements of the sparse hash XOR'd together, the
 * second element in the dense hash is the second sixteen elements of the sparse hash XOR'd 
 * together, etc.
 *
 ...
 * be between 0 and 255 (inclusive), always represent each number as two hexadecimal digits
 ...
 * @returns {number}
 */
function task2(input:string = day10input, listSize:number = 256):string {
  const inputs:number[] = input
    .split('')
    .map(char => char.charCodeAt(0))
    .concat(17, 31, 73, 47, 23);

  const list:number[] = runner(listSize, inputs, 64);
  
  const chunks:number[] = [];
  for (let i = 0; i < 256; i = i + 16) {
    /* tslint:disable-next-line:no-eval */
    chunks.push(eval(list.slice(i, i + 16).join(' ^ ')));
  }
  return chunks.map(n => n.toString(16)).map(x => x.length === 2 ? x : '0' + x).join('');
}


function createArray(size: number): number[] {
  const list:number[] = [];
  for (let i = 0; i < size; i = i + 1) { list.push(i); }
  return list;
}

function runner(listSize: number, lengths: number[], rounds: number = 1): number[] {
  const list = createArray(listSize);
  let position = 0;
  let skipSize = 0;
  for (let i = 0; i < rounds; i = i + 1) {
    for (const length of lengths) {
      const indexes = createArray(length).map(i => (position + i) % list.length);
      const segment = indexes.map(i => list[i]);
      segment.reverse();
