import { input as day5input } from './day5-input';
export { task1, task2 };
 ...
 * @returns {number}
 */
function task1(input:string = day5input):number {
 ...
 * @returns {number}
 */
function task2(input:string = day5input):number {
  const numbers:number[] = input.split('\n').map(v => +v);
  let position:number = 0;
  let steps:number = 0;
  while (true) {
    const offset = numbers[position];
    numbers[position] = numbers[position] + (offset < 3 ? 1 : -1);
    steps = steps + 1;
    position = position + offset;
    if (position < 0 || position >= numbers.length) {
      return steps;
