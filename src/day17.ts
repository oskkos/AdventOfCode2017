import { input as day17input } from './day17-input';
export { task1, task2 };
 ...
 */
function task1(moves:number = day17input, loops:number = 2018) {
  let state:number[];
  let currentPos:number;
  [state, currentPos] = runner(moves, 2018);
  return state[(currentPos + 1) % state.length];
}
function runner(moves:number, loops:number):[number[], number] {
  let currentPos:number = 0;
  const state:number[] = [0];
  for (let i = 1; i < loops; i = i + 1) {
    currentPos = ((currentPos + moves) % state.length) + 1;
 ...
 * 
 * The good news is that you have improved calculations for how to stop the spinlock. They
 *  indicate that you actually need to identify the value after 0 in the current state of the
 *  circular buffer.
 * 
 ...
 */
function task2(moves:number = day17input):number {
  let currentPos:number = 0;
  let length = 1;
  let result = 0;
  for (let i = 1; i < 50000000; i = i + 1) {
    currentPos = ((currentPos + moves) % length) + 1;
    length = length + 1;
    if (currentPos === 1) {
      result = i;
    }
